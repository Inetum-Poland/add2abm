...
printf 'Add2ABM script by Inetum\nRevision: %s\nWould you like to run it now? (y/n): ' ${REVISION}; read -r ANSWER
case "${ANSWER}" in
	[Yy]*) unset ANSWER; echo ;;
	*)     exit 0 ;;
esac
...
if ls "${USERS_PATH}"/*.bak &>/dev/null; then
	...
	printf 'Would you like to restart to macOS now? (y/n): '; read -r ANSWER
	case "${ANSWER}" in
		[Yy]*) printf 'Performing restart…'; reboot ;;
	esac
